"""Tenant Preference data model (Phase 4, Sprint 4.3).

The ``Preference`` model persists a tenant's apartment preferences. It is the
"Tenant Preference" entity (AGENTS 18, SYSTEM_REQUIREMENTS 7, 26) and carries the
structured attributes that later form the query vector ``U = (u1, ..., un)`` for
the Weighted KNN recommendation component (AGENTS 12). Each preference belongs
to a TENANT user (mirroring the ``Apartment.landlord`` relationship).

Facility preferences (parking, electricity, water, security, furnished) are
...
no preference on that facility (None). This explicit "no preference" state
supports the recommendation component's handling of missing values (AGENTS 27).
Numerical and text preferences are nullable/blank to leave the corresponding
feature unconstrained when the tenant has not expressed a preference.
"""
